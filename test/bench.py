...
def parse_args():
    ...
    p.add_argument("--requests", type=int, default=8, help="total number of requests")
    p.add_argument("--concurrency", type=int, default=8, help="concurrent in-flight requests")
    p.add_argument("--timeout", type=float, default=120, help="session total timeout (s) - allows for first-time compile")
...
async def one(sess, url: str, headers: dict, body: bytes):
    ...
    async with sess.post(url, data=body, headers=headers) as r:  # no per-request timeout
        r.raise_for_status()
        js = await r.json()
    return (time.perf_counter() - t0) * 1000.0, js


async def worker(num: int, url: str, headers: dict, body: bytes, timeout_total: float):
    results = []
...
async def main_async(args):
    ...
    if args.auth:
        headers["Authorization"] = f"Key {args.auth}"

    body = build_payload(args)

    workers = max(1, min(args.concurrency, args.requests))
    base, rem = divmod(args.requests, workers)
    counts = [base + (1 if i < rem else 0) for i in range(workers)]

    tasks = [
        asyncio.create_task(worker(counts[i], url, headers, body, args.timeout))
        for i in range(workers)
    ]
    nested = await asyncio.gather(*tasks)
    all_results = [item for sub in nested for item in sub]

    lat = sorted(x[0] for x in all_results)
    n = len(lat)
    p50 = lat[int(0.5 * n)] if n else 0.0
    p95 = lat[max(int(0.95 * n) - 1, 0)] if n else 0.0
    print(f"url={url} total={args.requests} conc={args.concurrency} p50={p50:.1f}ms p95={p95:.1f}ms")
    if all_results:
